"""
Improved RAG (Retrieval-Augmented Generation) System using LangChain
Key improvements:
    ...
    def load_documents(self, file_type: Optional[str] = None) -> List[Document]:
        ...
        try:
            if file_type == "txt":
                ...
                # For CSV, load directly
                import glob
                csv_files = glob.glob(os.path.join(self.documents_path, "**/*.csv"), recursive=True)
                documents = []
...
def main():
    ...
    config = RAGConfig(
        ...
        temperature=0,
        top_k=3,
        file_type="txt"  # Change to "csv" for CSV files
    )
